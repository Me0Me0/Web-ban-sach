...
fetch("/api/stores/" + storeID, options)
.then(data => data.json())
.then(data =>  { 
  document.title = data.__data__.name;
  document.getElementById("storename").innerHTML = data.__data__.name;
  document.querySelector("#store-detail-link").href = `/stores/${data.__data__.id}/details`;
})
.catch((err) => {
...
async function getProducts() {
    const res = await fetch(`/api/stores/${storeID}/products?limit=${limit * 5}&skip=${skip}`);
